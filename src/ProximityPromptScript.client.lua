...
local EnableAutomaticSizeVerticalOffsetWidthFix = false
local FFlagFixProximityPromptAncestry = true   -- or false; either is fine
...
local function createPrompt(prompt, inputType, gui)
	...
	table.insert(tweensForButtonHoldEnd, TweenService:Create(frame, tweenInfoFast, { BackgroundTransparency = 1 }))
	table.insert(tweensForFadeOut, TweenService:Create(frame, tweenInfoFast, { BackgroundTransparency = 1 }))
	table.insert(tweensForFadeIn, TweenService:Create(frame, tweenInfoFast, { BackgroundTransparency = 1 }))
